...
const imageToFile = async (image, option) => {
  const { quality, type, orientation } = { type: 'image/jpeg', quality: 1, orientation: 1, ...option }
  return new Promise((resolve, reject) => {
    typeOf(image) !== 'HTMLImageElement' && typeOf(image) !== 'HTMLCanvasElement' && reject(new Error('Image must be a element.'))
